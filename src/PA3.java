...
public class PA3 {
	...
	// A note:
	//		Professor Kardaras,
	//		Thank you for helping me with this assignment even after it was due.
	...
	public static void main(String[] args) throws IOException {
		...
		// Write the student info + scores to gradeData.txt
		for (int i = 0; i < student.size(); i++) {
			for (int k = 0; k < tests.size(); k++) {
				if (student.get(i).getID().equals(tests.get(k).getID())) {
					studentwriter.append((String.format("%-15s%-17s%-5s%5s", 
							student.get(i).getFirstName(), student.get(i).getLastName(), 
							df.format(tests.get(k).getScore()), tests.get(k).getGrade())));
					studentwriter.newLine();
				}
			}
		}
		...
		studentwriter.append(String.format("%-15s%-15s%-15s", "A ", calcAverages("A ", tests), 
				String.format("%.2f%%", (calcAverages("A ", tests)/tests.size()))));
		studentwriter.newLine();
		// ----
		...
		studentwriter.append(String.format("%-15s%-15s%-15s", "B+", calcAverages("B+", tests), 
				String.format("%.2f%%", (calcAverages("B+", tests)/tests.size()))));
		studentwriter.newLine();
		// ----
		...
		studentwriter.append(String.format("%-15s%-15s%-15s", "C+", calcAverages("C+", tests), 
				String.format("%.2f%%", (calcAverages("C+", tests)/tests.size()))));
		studentwriter.newLine();
		// ----
		...
		studentwriter.append(String.format("%-15s%-15s%-15s", "D ", calcAverages("D ", tests), 
				String.format("%.2f%%", (calcAverages("D ", tests)/tests.size()))));
		studentwriter.newLine();
		// ----
		
		// F ---
		studentwriter.append(String.format("%-15s%-15s%-15s", "F ", calcAverages("F ", tests), 
				String.format("%.2f%%", (calcAverages("F ", tests)/tests.size()))));
		studentwriter.newLine();
		// ----
